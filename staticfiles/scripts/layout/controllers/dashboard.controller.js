    ...
    function DashboardController($scope, $filter, $state, Authentication, Order, Messages) {
        ...
        function getOrderDetailsSuccess(data){
            ...
            angular.forEach(data, function(val, key){
                if(val.order_status === 'PEN'){
                    vm.orders.PEN.push(val.id);
                }else if(val.order_status === 'OFR'){
                    vm.orders.OFR.push(val.id);
                }else if(val.order_status === 'APV'){   
                    vm.orders.APV.push(val.id);
                }else if(val.order_status === 'INV'){
                    vm.orders.INV.push(val.id);
                }                
            })
        ...
        vm.goToItem = function(dash){
            ...
            }else if(dash.content_type === 'account'){
                $state.go('app.pages.profile', {username:dash.extra.account_username});
            }else{
